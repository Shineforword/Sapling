...
- (void)composeView
{
    ...
    [sendButton setBackgroundImage:[[UIImage imageWithColor:BASE_GREEN_COLOR] stretchableImageWithLeftCapWidth:3 topCapHeight:22] forState:UIControlStateNormal];
...
- (void)layoutSubviews
{
    ...
    dividLbl.backgroundColor = QYQCOLOR(227, 227, 229);
...
- (id) initWithFrame:(CGRect)frame andAboveView:(UIView *)bgView
{
    ...
    {
        /** 大背景图*/
        tapView = [[UIView alloc] initWithFrame:CGRectMake(0, 0,SCREEN_WIDTH, SCREEN_HEIGHT)];
...
- (void)sendButtonPressed:(id)sender{
    ...
    if([self.delegate respondsToSelector:@selector(YMReplyInputWithReply: appendIndexPath: andName:)]){
        
        [self.delegate YMReplyInputWithReply:textView.text appendIndexPath:self.indexPath andName:self.replayName];
    }
...
- (void)disappear{
    ...
    if([self.delegate respondsToSelector:@selector(destoryInputView)]){
        
        [self.delegate destoryInputView];
    }
