//
//  LSDetailMessageViewController.m
//  Sapling
//
...
@interface LSDetailMessageViewController ()
/** 头视图 */
...
@implementation LSDetailMessageViewController
...
- (void)initView{
    ...
    NSArray * valueArray = @[_kUser.uid ? _kUser.uid : @"",
                             _kUser.gender ? _kUser.gender : @"",
                             _kUser.email ? _kUser.email : @"",
                             _kUser.birthday ? _kUser.birthday : @""
                             ];
    /** 头视图 */
    {
...
{
    UIView * view = [[UIView alloc] initWithFrame:CGRectMake(0, 80 + index * 40, SCREEN_WIDTH, 40)];
...
-(void)confirmBut
{
    if (self.isFromChatVC == YES) {
        ...
        if (_kUser.is_contact || self.isFromContact) {
            LSChatViewController * chat = [[LSChatViewController alloc] init];
